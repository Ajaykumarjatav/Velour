        ...
        <form action="{{ route('services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            @if($assignedBusinessTypes->isEmpty())
                <div class="rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 px-4 py-3 text-sm text-amber-900 dark:text-amber-100">
                    Add at least one business type for this location under <a href="{{ route('settings.index') }}?tab=salon" class="font-medium underline">Settings → Salon</a> before you can assign a category to this service.
                </div>
            @else
            ...
            <div>
                ...
                <p class="form-hint mb-3">Home visits appear online only when <strong>Enable home visits</strong> is on under Settings → Salon.</p>
                <div class="space-y-2">
                    <label class="flex items-start gap-3 rounded-xl border border-gray-200 dark:border-gray-700 p-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 has-[:checked]:border-velour-400 dark:has-[:checked]:border-velour-600">
                        <input type="radio" name="service_location" value="onsite" class="mt-1 text-velour-600" {{ old('service_location', $service->service_location ?? 'onsite') === 'onsite' ? 'checked' : '' }} required>
                    ...
                    <label class="flex items-start gap-3 rounded-xl border border-gray-200 dark:border-gray-700 p-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 has-[:checked]:border-velour-400 dark:has-[:checked]:border-velour-600">
                        <input type="radio" name="service_location" value="home" class="mt-1 text-velour-600" {{ old('service_location', $service->service_location ?? 'onsite') === 'home' ? 'checked' : '' }}>
                        <span class="text-sm"><span class="font-medium text-heading">Home visit (client’s location)</span><span class="block text-muted text-xs mt-0.5">You or your team travel to the client. @if(!($currentSalon->home_services_enabled ?? false))<span class="text-amber-700 dark:text-amber-300"> Not shown on public booking until home visits are enabled.</span>@endif</span></span>
                    </label>
                </div>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="btn-primary flex-1 sm:flex-none" @if($assignedBusinessTypes->isEmpty()) disabled @endif>Save Changes</button>
...
<div id="inline-cat-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
        ...
        <div class="space-y-4">
            ...
            @php $modalDefaultBt = $service->category?->business_type_id ?? optional($assignedBusinessTypes->first())->id; @endphp
            <div>
                ...
                <select id="inline-cat-business-type" class="form-select">
                    @foreach($assignedBusinessTypes as $bt)
                        <option value="{{ $bt->id }}" {{ (string)($modalDefaultBt ?? '') === (string)$bt->id ? 'selected' : '' }}>{{ $bt->name }}</option>
                    @endforeach
                </select>
