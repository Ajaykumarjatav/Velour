...
<nav
    x-data="{
        ...
        menuOpen: false,
        init() {
            const onScroll = () => { this.isSticky = window.scrollY > 120 };
            onScroll();
            window.addEventListener('scroll', onScroll, { passive: true });
        }
    }"
    :class="isSticky
        ? 'fixed top-0 left-0 right-0 shadow-lg bg-[#F5ECE7]/95 backdrop-blur-md py-1'
        : 'relative bg-section-lightest py-3'"
    class="sf-sticky-nav w-full z-50 transition-all duration-300 border-b border-[#F5ECE7]"
>
    <div class="max-w-[1360px] mx-auto px-4">
        <div class="flex items-center justify-between py-2 lg:hidden">
            ...
            <button
                ...
                class="p-2.5 rounded-full hover:bg-gray-100 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#c98f8f]"
        ...
        >
            <div class="flex flex-col lg:flex-row items-center w-full lg:justify-center gap-1.5 lg:gap-8">
                ...
                <a
                    href="{{ $link['href'] }}"
                    @click="active = @js($link['label']); menuOpen = false"
                    :class="active === @js($link['label'])
