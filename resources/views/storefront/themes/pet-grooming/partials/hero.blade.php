@php
    ...
    if (!$salon) return;
    $heroImage = $salon['cover_image_url'] ?? $asset('Groomed Golden Retriever.png');
...
>
    ...
    <div class="absolute left-[-160px] top-[120px] w-[520px] h-[520px] rounded-full border border-[#b1adad] opacity-10 hidden lg:block"></div>
    <div class="absolute left-[-15px] top-[640px] w-[187px] h-[349px] border border-[#b1adad] rounded-br-[180px] opacity-10 hidden lg:block"></div>
    ...
    <div class="relative z-10 max-w-[1360px] mx-auto px-4 pt-16 md:pt-24 lg:pt-[80px] pb-24">
        <div class="max-w-[800px] text-center lg:text-left">
            <div
                ...
                style="background: linear-gradient(93deg, #b1a1dad 2.49%, #b1adad 96.02%, #b1adad 184.57%); border: 0.72px solid #b1adad;"
            >
                ...
                @foreach(range(1, 3) as $i)
                    <div class="flex flex-col items-center lg:items-start gap-3 group/item cursor-default">
                        <div class="w-[50px] h-[50px] rounded-full bg-white/5 border border-white/10 flex items-center justify-center transition-all duration-300 group-hover/item:bg-primary/20 group-hover/item:border-primary/50 group-hover/item:scale-105">
                            <img src="{{ $asset('animal (1) 3.png') }}" alt="" />
                        </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 md:gap-6 w-full sm:w-auto">
                @include('storefront.partials.book-button', [
                    'class' => 'w-full sm:w-auto inline-flex items-center justify-center gap-2.5 bg-[#7a8b72] hover:bg-[#6a7b62] text-white font-semibold text-sm md:text-base rounded-full px-8 py-4 transition-all duration-300 shadow-md hover:shadow-[#7a8b72]/20 hover:scale-[1.02] active:scale-[0.98]',
