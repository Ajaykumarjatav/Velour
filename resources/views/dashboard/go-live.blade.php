...
@push('styles')
<style>
  .stat-card   { @apply bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-shadow; }
  .tab-btn     { @apply px-4 py-2 text-sm font-medium rounded-xl transition-all; }
  .tab-btn.on  { @apply bg-amber-500 text-white shadow-sm; }
  .tab-btn.off { @apply text-gray-500 hover:text-gray-700 hover:bg-gray-100; }
  ...
  .badge-high  { @apply bg-red-50 text-red-600 border border-red-100 text-xs px-2 py-0.5 rounded-full font-medium; }
  .badge-med   { @apply bg-amber-50 text-amber-600 border border-amber-100 text-xs px-2 py-0.5 rounded-full font-medium; }
  .badge-low   { @apply bg-gray-100 text-gray-500 border border-gray-200 text-xs px-2 py-0.5 rounded-full font-medium; }
...
>
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-2">
    ...
    <div class="flex items-center gap-3 bg-white border border-gray-100 rounded-2xl px-5 py-3 shadow-sm">
      <div>
        <p class="text-sm font-semibold text-gray-800">Online Booking</p>
        <p class="text-xs text-gray-400" x-text="salon.online_booking_enabled ? 'Clients can book right now' : 'Booking page is hidden'"></p>
      </div>
      <button
        @click="toggleBooking()"
  ...
  <div x-show="checklist.score < 100" x-cloak
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-50">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="font-semibold text-gray-800 text-sm">Go-Live Readiness</h2>
          <p class="text-xs text-gray-400" x-text="`${checklist.done} of ${checklist.total} steps complete`"></p>
        </div>
      </div>
      {{-- Progress pill --}}
      <div class="flex items-center gap-3">
        <div class="hidden sm:block w-36 bg-gray-100 rounded-full h-2">
    ...
    <ul class="divide-y divide-gray-50 px-0">
      <template x-for="item in checklist.items" :key="item.key">
        <li class="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition">
          {{-- Status icon --}}
          <div :class="item.done
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-gray-800" x-text="item.label"
               :class="item.done ? 'line-through text-gray-400' : ''"></p>
            <p class="text-xs text-gray-400 truncate" x-text="item.tip" x-show="!item.done"></p>
          </div>
          {{-- Priority badge --}}
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="stat-card">
      <div class="flex items-center justify-between mb-3">
        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Visits</p>
        <span class="text-xl">👁</span>
      </div>
      <p class="text-2xl font-bold text-gray-900" x-text="stats.link_visits ?? '—'"></p>
    ...
    <div class="stat-card">
      ...
      <p class="text-2xl font-bold text-gray-900" x-text="(stats.conversion_rate ?? 0) + '%'"></p>
      <p class="text-xs text-gray-400 mt-1" x-text="(stats.conversions ?? 0) + ' bookers this month'"></p>
    </div>

    {{-- Online bookings --}}
    <div class="stat-card">
      <div class="flex items-center justify-between mb-3">
        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Online Bookings</p>
        <span class="text-xl">📅</span>
      </div>
      <p class="text-2xl font-bold text-gray-900" x-text="stats.online_bookings ?? '—'"></p>
      <p class="text-xs text-gray-400 mt-1">from your booking page</p>
    </div>

    {{-- Online revenue --}}
    <div class="stat-card">
      <div class="flex items-center justify-between mb-3">
        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Online Revenue</p>
        <span class="text-xl">💷</span>
      </div>
      <p class="text-2xl font-bold text-gray-900"
         x-text="'£' + Number(stats.online_revenue ?? 0).toLocaleString('en-GB', {minimumFractionDigits:2})">
      </p>
      <p class="text-xs text-gray-400 mt-1" x-text="stats.period ?? ''"></p>
    </div>
    ...
    <div class="lg:col-span-3 space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-2">
          <span class="text-lg">🔗</span>
          <h2 class="font-semibold text-gray-800">Your Booking Link</h2>
        </div>
        <div class="p-6 space-y-4">
          {{-- URL bar --}}
          <div class="flex gap-2">
            <div class="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 gap-2 min-w-0">
              <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/>
              </svg>
              <a :href="bookingUrl" target="_blank" rel="noopener"
                 class="text-sm text-amber-700 font-medium truncate hover:underline"
                 x-text="bookingUrl"></a>
            </div>
            <button @click="copyUrl(bookingUrl, 'main')"
              class="copy-btn border-gray-200 text-gray-600 hover:border-amber-300 hover:text-amber-600 flex-shrink-0">
          ...
          {{-- UTM link builder --}}
          <details class="group">
            <summary class="text-xs text-gray-400 cursor-pointer hover:text-amber-600 transition select-none list-none flex items-center gap-1">
            ...
            <div class="mt-3 grid sm:grid-cols-3 gap-2" x-data="{ source:'', medium:'', campaign:'' }">
              <input x-model="source"   placeholder="utm_source (e.g. instagram)"  class="border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 focus:ring-2 focus:ring-amber-300 outline-none">
              <input x-model="medium"   placeholder="utm_medium (e.g. bio)"        class="border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 focus:ring-2 focus:ring-amber-300 outline-none">
              <input x-model="campaign" placeholder="utm_campaign (e.g. summer25)"  class="border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 focus:ring-2 focus:ring-amber-300 outline-none">
              <div class="sm:col-span-3 flex gap-2 mt-1">
                <div class="flex-1 bg-gray-50 border border-gray-100 rounded-lg px-3 py-2 text-xs text-gray-500 truncate font-mono"
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="text-lg">📣</span>
            <h2 class="font-semibold text-gray-800">Share on Social</h2>
          </div>
          <span class="text-xs text-gray-400">Click counts this month</span>
        </div>
        <div class="p-6">
          ...
          <div class="mt-4 flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
            ...
            <button @click="copyUrl(bookingUrl, 'social'); trackClick('copy_link')"
              class="copy-btn border-gray-200 text-gray-600 hover:border-amber-300 hover:text-amber-600">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-2">
          <span class="text-lg">🖥</span>
          <h2 class="font-semibold text-gray-800">Embed on Your Website</h2>
        </div>
        <div class="p-6 space-y-4">
          {{-- Tab switcher --}}
          <div class="flex gap-1.5 bg-gray-100 p-1 rounded-xl w-fit">
    ...
    <div class="lg:col-span-2 space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-50 flex items-center gap-2">
          <span class="text-lg">📱</span>
          <h2 class="font-semibold text-gray-800">QR Code</h2>
        </div>
        <div class="p-5 flex flex-col items-center gap-4">
          <div class="bg-white border-2 border-gray-100 rounded-2xl p-3 shadow-inner">
          ...
          <div class="w-full space-y-2">
            <a :href="qrUrl + '&format=png'" download="velour-booking-qr.png"
              ...
              class="flex items-center justify-center gap-2 w-full bg-gray-900 text-white text-sm font-semibold py-2.5 rounded-xl hover:bg-gray-700 transition">
              ⬇ Download PNG
            </a>
            <p class="text-xs text-gray-400 text-center">Print on receipts, menus, windows &amp; marketing materials</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-gray-800">Booking Settings</h2>
          </div>
          ...
          <template x-for="setting in bookingSettings" :key="setting.key">
            <div class="flex items-center justify-between px-5 py-3.5">
              <div class="flex-1 pr-4">
                <p class="text-sm font-medium text-gray-800" x-text="setting.label"></p>
                <p class="text-xs text-gray-400 mt-0.5" x-text="setting.description"></p>
              </div>
              <template x-if="setting.type === 'toggle'">
                <button
                  @click="saveSetting(setting.key, !salon[setting.key])"
                  :class="salon[setting.key] ? 'bg-green-500' : 'bg-gray-300'"
              ...
              <template x-if="setting.type === 'number'">
                <input
                  ...
                  class="w-20 text-right text-sm border border-gray-200 rounded-lg px-2 py-1.5 focus:ring-2 focus:ring-amber-300 outline-none"
                >
              </template>
            </div>
          </template>
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="text-lg">📊</span>
            <h2 class="font-semibold text-gray-800">Traffic Sources</h2>
          </div>
          ...
          <template x-for="src in sources.slice(0,5)" :key="src.source">
            <div class="space-y-1">
              <div class="flex items-center justify-between text-xs">
                ...
                <div class="flex items-center gap-2">
                  <span class="text-gray-400" x-text="src.visits + ' visits'"></span>
        ...
        <div class="px-5 py-8 text-center text-gray-300" x-show="sources.length === 0 && !loading">
  ...
  {{-- ── 30-DAY TREND CHART ──────────────────────────────────────────────── --}}
  <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
      <div class="flex items-center gap-2">
        <span class="text-lg">📈</span>
        <h2 class="font-semibold text-gray-800">30-Day Trend</h2>
      </div>
    ...
    <div class="p-6">
      ...
      <div class="relative h-48 w-full" x-show="trend.length > 0">
        <svg class="w-full h-full" viewBox="0 0 800 180" preserveAspectRatio="none">
          ...
          {{-- Grid lines --}}
          <line x1="0" y1="45"  x2="800" y2="45"  stroke="#f3f4f6" stroke-width="1"/>
          <line x1="0" y1="90"  x2="800" y2="90"  stroke="#f3f4f6" stroke-width="1"/>
          <line x1="0" y1="135" x2="800" y2="135" stroke="#f3f4f6" stroke-width="1"/>
          <line x1="0" y1="180" x2="800" y2="180" stroke="#f3f4f6" stroke-width="1"/>

          {{-- Visits line (amber) --}}
          <polyline
            :points="buildPolyline(trend, 'visits', 800, 180)"
            class="sparkline"
            stroke="#f59e0b"
            stroke-width="2.5"
            fill="none"
          ...
          {{-- Bookings line (violet) --}}
          <polyline
            :points="buildPolyline(trend, 'bookings', 800, 180)"
            class="sparkline"
            stroke="#8b5cf6"
            stroke-width="2.5"
            fill="none"
        ...
        <div class="flex justify-between mt-1 px-1">
          <template x-for="(day, i) in trend" :key="i">
            <span x-show="i % 5 === 0 || i === trend.length - 1"
                  class="text-[10px] text-gray-300" x-text="day.label"></span>
          </template>
        </div>
      </div>

      {{-- Empty state --}}
      <div class="h-48 flex items-center justify-center text-gray-300" x-show="trend.length === 0 && !loading">
      ...
      {{-- Summary row --}}
      <div class="grid grid-cols-3 gap-4 mt-6 pt-4 border-t border-gray-50" x-show="trend.length > 0">
        <div class="text-center">
          <p class="text-lg font-bold text-gray-900" x-text="trend.reduce((a,d) => a + d.visits, 0)"></p>
          <p class="text-xs text-gray-400">Total visits (30d)</p>
        </div>
        <div class="text-center">
          <p class="text-lg font-bold text-gray-900" x-text="trend.reduce((a,d) => a + d.bookings, 0)"></p>
          <p class="text-xs text-gray-400">Online bookings</p>
        </div>
        <div class="text-center">
          <p class="text-lg font-bold text-gray-900" x-text="
            (() => {
          ...
          "></p>
          <p class="text-xs text-gray-400">Avg conversion</p>
        </div>
      </div>
  ...
  <div class="grid sm:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-2">
        <span class="text-lg">📱</span>
        <h2 class="font-semibold text-gray-800">Visitors by Device</h2>
      </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1">
              <div class="flex justify-between text-xs mb-1">
                <span class="font-medium text-gray-700 capitalize" x-text="dev.device"></span>
                <span class="text-gray-500" x-text="dev.count + ' (' + dev.percentage + '%)'"></span>
              </div>
      ...
      <div class="px-6 py-8 text-center text-gray-300" x-show="devices.length === 0 && !loading">
        <p class="text-sm">No device data yet</p>
      </div>
    </div>

    {{-- ── PRO TIPS ──────────────────────────────────────────────────────── --}}
    <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border border-amber-100 shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-amber-100 flex items-center gap-2">
        <span class="text-lg">💡</span>
        <h2 class="font-semibold text-amber-800">Growth Tips</h2>
      </div>
      <ul class="p-6 space-y-3">
        <li class="flex items-start gap-3 text-sm text-amber-900">
          <span class="flex-shrink-0 w-5 h-5 bg-amber-200 rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5">1</span>
        ...
        <li class="flex items-start gap-3 text-sm text-amber-900">
          <span class="flex-shrink-0 w-5 h-5 bg-amber-200 rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5">2</span>
        ...
        <li class="flex items-start gap-3 text-sm text-amber-900">
          <span class="flex-shrink-0 w-5 h-5 bg-amber-200 rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5">3</span>
          <span><strong>Window QR sticker</strong> — Print your QR code and put it in your window. Walk-bys become bookings.</span>
        </li>
        <li class="flex items-start gap-3 text-sm text-amber-900">
          <span class="flex-shrink-0 w-5 h-5 bg-amber-200 rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5">4</span>
