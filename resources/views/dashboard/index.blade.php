...
@php
    ...
    $dashboardBookingUrl = $salon ? \App\Support\StorefrontUrl::booking($salon) : '';
    $dashboardWaShareUrl = $dashboardBookingUrl !== ''
        ? 'https://wa.me/?text=' . rawurlencode('Book your next appointment with us!' . "\n" . $dashboardBookingUrl)
        : '';
@endphp
<div x-data="analyticsSlider({{ $analyticsWidgetsJson }}, {{ $detailListsJson }}, {{ $periodBoundsJson }})" class="mb-7">
    {{-- Period tabs + WhatsApp share --}}
    <div class="flex items-center justify-between gap-3 mb-4 flex-wrap">
        ...
        @if($dashboardWaShareUrl !== '' && empty($stylistDashboardScoped))
        <a href="{{ $dashboardWaShareUrl }}"
           target="_blank"
           rel="noopener noreferrer"
           class="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold rounded-full border border-emerald-300/80 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-colors shrink-0"
