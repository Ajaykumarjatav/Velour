    ...
    <div class="card p-6">
        <form action="{{ route('inventory.update', $item->id) }}" method="POST" class="space-y-5">
            ...
            <x-relation-field-with-create
                ...
                select-id="inv-edit-category"
                type="inventory_category"
                :required="false"
                error-name="inventory_category_id">
                <option value="">No category</option>
                @foreach($categories as $cat)
                <option value="{{ $cat->id }}" @selected(old('inventory_category_id', $item->category_id) == $cat->id)>{{ $cat->name }}</option>
                @endforeach
            </x-relation-field-with-create>
            <div>
            ...
            <div class="flex items-end gap-2">
                <div class="flex-1 min-w-0">
                    <label class="form-label" for="inv-edit-supplier">Supplier</label>
                    <select name="supplier" id="inv-edit-supplier"
                            class="form-select w-full @error('supplier') form-input-error @enderror">
                        <option value="">No supplier</option>
                        @foreach($suppliers as $s)
                        <option value="{{ $s }}" @selected(old('supplier', $item->supplier) === $s)>{{ $s }}</option>
                        @endforeach
                    </select>
                    @error('supplier')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <x-relation-quick-create-trigger type="inventory_supplier" select-id="inv-edit-supplier" />
            </div>
