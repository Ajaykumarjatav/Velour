...
- Name: admin-pages.spec.ts >> Super Admin pages (plan SA-*) >> SA-09h GET /admin/tenants/2/reviews
- Location: tests\playwright\admin-pages.spec.ts:44:5

# Error details

```
Error: SA-09h admin/tenants/2/reviews status
  ...
  - main [ref=e16]:
    - generic [ref=e17]:
      ...
      - generic [ref=e810]:
        ...
        - generic [ref=e812]: GET /admin/tenants/2/reviews
      ...
      - generic [ref=e896]:
        ...
        - generic [ref=e899]:
          - generic [ref=e900]:
            ...
            - code [ref=e904]: App\Http\Controllers\Admin\Tenant\AdminTenantReviewController@index
          - generic [ref=e905]:
            - generic [ref=e906] [cursor=pointer]: route name
            - code [ref=e909]: admin.tenants.reviews.index
          - generic [ref=e910]:
        ...
        - generic [ref=e922]:
          - generic [ref=e923]:
            - generic [ref=e924]:
              - text: mysql
              - generic [ref=e925]: (6.04 ms)
            - code [ref=e928]: "select * from `users` where `id` = 1 and `users`.`deleted_at` is null limit 1"
          - generic [ref=e929]:
            - generic [ref=e930]:
              - text: mysql
              - generic [ref=e931]: (0.61 ms)
            - code [ref=e934]: "select * from `salons` where `salons`.`id` = 2 limit 1"
          - generic [ref=e935]:
            - generic [ref=e936]:
              - text: mysql
              - generic [ref=e937]: (11.26 ms)
            - code [ref=e940]: "insert into `audit_logs` (`user_id`, `user_email`, `user_name`, `salon_id`, `event`, `event_category`, `severity`, `description`, `subject_type`, `subject_id`, `metadata`, `ip_address`, `user_agent`, `session_id`, `request_id`, `http_method`, `url`, `occurred_at`) values (1, 'admin@velour.app', 'Velour Admin', 2, 'admin.tenant.view', 'admin', 'warning', 'Admin viewed reviews.index list for salon 'ak salon'', 'App\\Models\\Salon', 2, '{\"module\":\"reviews.index\",\"salon_id\":2}', '::1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.7922.34 Safari/537.36', 'EVylLVZnikIxF2MdWsshwm23DmppHzyCjf8oYXN1', NULL, 'GET', 'http://localhost/admin/tenants/2/reviews', '2026-08-09 12:37:12')"
          - generic [ref=e941]:
            - generic [ref=e942]:
              - text: mysql
              - generic [ref=e943]: (0.3 ms)
            - code [ref=e946]: "select count(*) as aggregate from `reviews` where `salon_id` = 2"
          - generic [ref=e947]:
            - generic [ref=e948]:
              - text: mysql
              - generic [ref=e949]: (1.02 ms)
            - code [ref=e952]: "select * from `reviews` where `salon_id` = 2 order by `created_at` desc limit 25 offset 0"
          - generic [ref=e953]:
            - generic [ref=e954]:
              - text: mysql
              - generic [ref=e955]: (0.37 ms)
