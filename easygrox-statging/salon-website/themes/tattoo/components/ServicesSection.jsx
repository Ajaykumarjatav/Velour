...
export default function ServicesSection() {
  ...
  return (
    <section id="services" className="w-full bg-section-light py-20 lg:py-24 overflow-hidden">
      <div className="max-w-[1360px] mx-auto px-4 min-w-0">
        ...
        {services.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 lg:gap-x-8 gap-y-2">
              ...
              >
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-[50px] h-[50px] md:w-[60px] md:h-[60px] flex-shrink-0 flex items-center justify-center bg-gray-50 rounded-lg p-1.5 transition-transform duration-300 group-hover/item:scale-105">
                    <img src={assetUrl('assets/equipment 1.png')} alt="" className="w-full h-full object-contain" />
                  </div>
                  <div className="flex flex-col gap-1 min-w-0">
                    <span className="font-manrope font-bold text-base md:text-lg text-black transition-colors duration-300 group-hover/item:text-primary truncate">{service.name}</span>
                    <span className="font-inter font-normal text-xs md:text-sm text-text-muted line-clamp-2">
                      {service.description || `${service.duration_minutes} min`}
                    </span>
                  </div>
                </div>
                <span className="font-manrope font-bold text-xl md:text-2xl text-black whitespace-nowrap shrink-0">
          ...
          <p className="text-center text-text-muted font-inter text-sm py-8">
