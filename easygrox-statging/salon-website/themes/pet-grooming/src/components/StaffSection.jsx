...
function StaffServicesList({ member }) {
  ...
  return (
    <div className="mt-8 flex flex-col flex-1 justify-between">
      <p className="font-manrope font-bold text-lg md:text-xl text-black mb-2 transition-colors duration-300 group-hover:text-[#7a8b72]">
    ...
    <section id="staff" className="w-[120px] h-[120px] rounded-full overflow-hidden relative z-10 border-4 border-white shadow-md transition-all duration-300 group-hover:scale-105 group-hover:shadow-lg select-none">
      <div className="w-full h-full object-cover">
        ...
        {staff.length === 0 ? (
          ...
          <HorizontalDragScroll ariaLabel="Our staff" gapClass="gap-6 md:gap-8" className="inline-block text-[#7a8b72] hover:text-[#7a8b72]-dark font-manrope font-bold text-xs uppercase tracking-wider transition-colors duration-300 group/link">
            {staff.map((member) => (
              <article
                key={member.id}
                className="block h-0.5 w-0 bg-[#7a8b72]group-hover/link:w-full transition-all duration-300 mx-auto mt-0.5"
              >
