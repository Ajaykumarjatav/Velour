...
- Name: admin-pages.spec.ts >> Super Admin pages (plan SA-*) >> SA-09c GET /admin/tenants/2/staff
- Location: tests\playwright\admin-pages.spec.ts:44:5

# Error details

```
Error: SA-09c admin/tenants/2/staff status
  ...
  - main [ref=e16]:
    - generic [ref=e17]:
      ...
      - generic [ref=e810]:
        ...
        - generic [ref=e812]: GET /admin/tenants/2/staff
      ...
      - generic [ref=e896]:
        ...
        - generic [ref=e899]:
          - generic [ref=e900]:
            ...
            - code [ref=e904]: App\Http\Controllers\Admin\Tenant\AdminTenantStaffController@index
          - generic [ref=e905]:
            - generic [ref=e906] [cursor=pointer]: route name
            - code [ref=e909]: admin.tenants.staff.index
          - generic [ref=e910]:
        ...
        - generic [ref=e922]:
          - generic [ref=e923]:
            - generic [ref=e924]:
              - text: mysql
              - generic [ref=e925]: (7.99 ms)
            - code [ref=e928]: "select * from `users` where `id` = 1 and `users`.`deleted_at` is null limit 1"
          - generic [ref=e929]:
            - generic [ref=e930]:
              - text: mysql
              - generic [ref=e931]: (1.1 ms)
            - code [ref=e934]: "select * from `salons` where `salons`.`id` = 2 limit 1"
          - generic [ref=e935]:
            - generic [ref=e936]:
              - text: mysql
              - generic [ref=e937]: (19.93 ms)
            - code [ref=e940]: "insert into `audit_logs` (`user_id`, `user_email`, `user_name`, `salon_id`, `event`, `event_category`, `severity`, `description`, `subject_type`, `subject_id`, `metadata`, `ip_address`, `user_agent`, `session_id`, `request_id`, `http_method`, `url`, `occurred_at`) values (1, 'admin@velour.app', 'Velour Admin', 2, 'admin.tenant.view', 'admin', 'warning', 'Admin viewed staff.index list for salon 'ak salon'', 'App\\Models\\Salon', 2, '{\"module\":\"staff.index\",\"salon_id\":2}', '::1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.7922.34 Safari/537.36', 'EVylLVZnikIxF2MdWsshwm23DmppHzyCjf8oYXN1', NULL, 'GET', 'http://localhost/admin/tenants/2/staff', '2026-08-09 12:36:12')"
          - generic [ref=e941]:
            - generic [ref=e942]:
              - text: mysql
              - generic [ref=e943]: (1.28 ms)
            - code [ref=e946]: "select count(*) as aggregate from `staff` where `salon_id` = 2"
          - generic [ref=e947]:
            - generic [ref=e948]:
              - text: mysql
              - generic [ref=e949]: (1.06 ms)
            - code [ref=e952]: "select * from `staff` where `salon_id` = 2 order by `first_name` asc limit 25 offset 0"
          - generic [ref=e953]:
            - generic [ref=e954]:
              - text: mysql
              - generic [ref=e955]: (1.34 ms)
          ...
          - generic [ref=e1331]:
            - generic [ref=e1332]:
              - text: mysql
              - generic [ref=e1333]: (0.64 ms)
            - code [ref=e1336]: "select `salon_id` from `staff` where `user_id` = 1 and `deleted_at` is null limit 1"
          - generic [ref=e1337]:
            - generic [ref=e1338]:
              - text: mysql
              - generic [ref=e1339]: (0.71 ms)
            - code [ref=e1342]: "select * from `users` where `users`.`id` = 5 and `users`.`deleted_at` is null limit 1"
          - generic [ref=e1343]:
            - generic [ref=e1344]:
              - text: mysql
              - generic [ref=e1345]: (0.57 ms)
          ...
          - generic [ref=e1349]:
            - generic [ref=e1350]:
              - text: mysql
              - generic [ref=e1351]: (0.44 ms)
            - code [ref=e1354]: "select `salon_id` from `staff` where `user_id` = 1 and `deleted_at` is null limit 1"
          - generic [ref=e1355]:
            - generic [ref=e1356]:
              - text: mysql
              - generic [ref=e1357]: (0.62 ms)
            - code [ref=e1360]: "select count(*) as aggregate from `support_tickets` where `status` in ('open', 'in_progress') and `assigned_to` is null"
          - generic [ref=e1361]:
            - generic [ref=e1362]:
              - text: mysql
              - generic [ref=e1363]: (0.68 ms)
