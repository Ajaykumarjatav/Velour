...
- Name: admin-pages.spec.ts >> Super Admin pages (plan SA-*) >> SA-09f GET /admin/tenants/2/inventory
- Location: tests\playwright\admin-pages.spec.ts:44:5

# Error details

```
Error: SA-09f admin/tenants/2/inventory status
  ...
  - main [ref=e16]:
    - generic [ref=e17]:
      ...
      - generic [ref=e810]:
        ...
        - generic [ref=e812]: GET /admin/tenants/2/inventory
      ...
      - generic [ref=e896]:
        ...
        - generic [ref=e899]:
          - generic [ref=e900]:
            ...
            - code [ref=e904]: App\Http\Controllers\Admin\Tenant\AdminTenantInventoryController@index
          - generic [ref=e905]:
            - generic [ref=e906] [cursor=pointer]: route name
            - code [ref=e909]: admin.tenants.inventory.index
          - generic [ref=e910]:
        ...
        - generic [ref=e922]:
          - generic [ref=e923]:
            - generic [ref=e924]:
              - text: mysql
              - generic [ref=e925]: (3.66 ms)
            - code [ref=e928]: "select * from `users` where `id` = 1 and `users`.`deleted_at` is null limit 1"
          - generic [ref=e929]:
            - generic [ref=e930]:
              - text: mysql
              - generic [ref=e931]: (0.68 ms)
            - code [ref=e934]: "select * from `salons` where `salons`.`id` = 2 limit 1"
          - generic [ref=e935]:
            - generic [ref=e936]:
              - text: mysql
              - generic [ref=e937]: (11.33 ms)
            - code [ref=e940]: "insert into `audit_logs` (`user_id`, `user_email`, `user_name`, `salon_id`, `event`, `event_category`, `severity`, `description`, `subject_type`, `subject_id`, `metadata`, `ip_address`, `user_agent`, `session_id`, `request_id`, `http_method`, `url`, `occurred_at`) values (1, 'admin@velour.app', 'Velour Admin', 2, 'admin.tenant.view', 'admin', 'warning', 'Admin viewed inventory.index list for salon 'ak salon'', 'App\\Models\\Salon', 2, '{\"module\":\"inventory.index\",\"salon_id\":2}', '::1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.7922.34 Safari/537.36', 'EVylLVZnikIxF2MdWsshwm23DmppHzyCjf8oYXN1', NULL, 'GET', 'http://localhost/admin/tenants/2/inventory', '2026-08-09 12:36:50')"
          - generic [ref=e941]:
            - generic [ref=e942]:
              - text: mysql
              - generic [ref=e943]: (0.45 ms)
            - code [ref=e946]: "select count(*) as aggregate from `inventory_items` where `salon_id` = 2"
          - generic [ref=e947]:
            - generic [ref=e948]:
              - text: mysql
              - generic [ref=e949]: (0.93 ms)
            - code [ref=e952]: "select * from `inventory_items` where `salon_id` = 2 order by `name` asc limit 25 offset 0"
          - generic [ref=e953]:
            - generic [ref=e954]:
              - text: mysql
              - generic [ref=e955]: (0.46 ms)
          ...
          - generic [ref=e1043]:
            - generic [ref=e1044]:
              - text: mysql
              - generic [ref=e1045]: (0.33 ms)
            - code [ref=e1048]: "select `salon_id` from `staff` where `user_id` = 1 and `deleted_at` is null limit 1"
          - generic [ref=e1049]:
            - generic [ref=e1050]:
              - text: mysql
              - generic [ref=e1051]: (1.12 ms)
            - code [ref=e1054]: "select * from `users` where `users`.`id` = 5 and `users`.`deleted_at` is null limit 1"
          - generic [ref=e1055]:
            - generic [ref=e1056]:
              - text: mysql
              - generic [ref=e1057]: (0.58 ms)
          ...
          - generic [ref=e1061]:
            - generic [ref=e1062]:
              - text: mysql
              - generic [ref=e1063]: (0.65 ms)
            - code [ref=e1066]: "select `salon_id` from `staff` where `user_id` = 1 and `deleted_at` is null limit 1"
          - generic [ref=e1067]:
            - generic [ref=e1068]:
              - text: mysql
              - generic [ref=e1069]: (0.49 ms)
            - code [ref=e1072]: "select count(*) as aggregate from `support_tickets` where `status` in ('open', 'in_progress') and `assigned_to` is null"
          - generic [ref=e1073]:
            - generic [ref=e1074]:
              - text: mysql
