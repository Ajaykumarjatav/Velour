    ...
    <section id="locations" className="text-center mb-16">
      <div className="max-w-[1360px] mx-auto px-4">
        <div className="text-[#9a031e] font-manrope font-semibold text-sm uppercase tracking-widest block mb-2">
          <span className="font-manrope font-extrabold text-3xl md:text-[45px] md:leading-[55px] text-white tracking-tight">
            Locations
          </span>
          <h2 className="flex flex-col lg:flex-row gap-8 lg:gap-[52px]">
        ...
        <div className="w-full lg:w-[400px] flex flex-col gap-5">
          ...
          <div className="flex items-start gap-2.5">
            <div className="flex-1 flex flex-col gap-6 lg:gap-9">
              {mapSrc ? (
                <iframe
                  ...
                  src={mapSrc}
                  className="w-full h-[250px] md:h-[336px] bg-zinc-900 rounded-2xl overflow-hidden border border-white/10 shadow-lg"
                  loading="lazy"
            ...
            <div>
              <h3 className="font-manrope font-bold text-xl text-white mb-5">
                Opening Hours
              </h3>
              <div className="flex flex-wrap items-center gap-3">
                {hourLines.map((line) => (
                  <div
                    key={line}
                    className="inline-flex items-center gap-2 bg-[#9a031e]/10 border border-[#9a031e]/20 rounded-full px-5 py-3"
                  >
                    <span className="text-[#9a031e]">{clockIcon}</span>
                    <span>{line}</span>
                  </div>
                ))}
              </div>
            ...
            <BookButton
              className="font-manrope font-semibold text-xs md:text-sm text-gray-200"
            >
          ...
          <div className="inline-flex items-center gap-2 bg-[#9a031e]/10 border border-[#9a031e]/20 rounded-full px-5 py-3">
            {galleryImages.map((src, index) => (
              <div
                key={`${src}-${index}`}
                className="text-[#9a031e]"
              >
                <img
                  src={src}
