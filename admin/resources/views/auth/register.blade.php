...
<form action="{{ route('register.submit') }}" method="POST" class="auth-form" id="register-form"
    ...
    <div class="auth-grid">
        ...
        <div class="auth-field auth-field--full">
            ...
            <input id="register-business-name" type="text" name="salon_name" value="{{ old('salon_name') }}" required maxlength="150" autocomplete="organization" placeholder="Maison Lumière Salon"
                   class="auth-input @error('salon_name') is-invalid @enderror">
            @error('salon_name')<p class="auth-error">{{ $message }}</p>@enderror
        </div>
        <div class="auth-field auth-field--full">
