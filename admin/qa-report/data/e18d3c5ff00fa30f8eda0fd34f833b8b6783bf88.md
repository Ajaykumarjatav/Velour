...
- Name: admin-pages.spec.ts >> Super Admin pages (plan SA-*) >> SA-09e GET /admin/tenants/2/services
- Location: tests\playwright\admin-pages.spec.ts:44:5

# Error details

```
Error: SA-09e admin/tenants/2/services status
  ...
  - main [ref=e16]:
    - generic [ref=e17]:
      ...
      - generic [ref=e810]:
        ...
        - generic [ref=e812]: GET /admin/tenants/2/services
      ...
      - generic [ref=e896]:
        ...
        - generic [ref=e899]:
          - generic [ref=e900]:
            ...
            - code [ref=e904]: App\Http\Controllers\Admin\Tenant\AdminTenantServiceController@index
          - generic [ref=e905]:
            - generic [ref=e906] [cursor=pointer]: route name
            - code [ref=e909]: admin.tenants.services.index
          - generic [ref=e910]:
        ...
        - generic [ref=e922]:
          - generic [ref=e923]:
            - generic [ref=e924]:
              - text: mysql
              - generic [ref=e925]: (7.35 ms)
            - code [ref=e928]: "select * from `users` where `id` = 1 and `users`.`deleted_at` is null limit 1"
          - generic [ref=e929]:
            - generic [ref=e930]:
              - text: mysql
              - generic [ref=e931]: (3.39 ms)
            - code [ref=e934]: "select * from `salons` where `salons`.`id` = 2 limit 1"
          - generic [ref=e935]:
            - generic [ref=e936]:
              - text: mysql
              - generic [ref=e937]: (5.83 ms)
            - code [ref=e940]: "insert into `audit_logs` (`user_id`, `user_email`, `user_name`, `salon_id`, `event`, `event_category`, `severity`, `description`, `subject_type`, `subject_id`, `metadata`, `ip_address`, `user_agent`, `session_id`, `request_id`, `http_method`, `url`, `occurred_at`) values (1, 'admin@velour.app', 'Velour Admin', 2, 'admin.tenant.view', 'admin', 'warning', 'Admin viewed services.index list for salon 'ak salon'', 'App\\Models\\Salon', 2, '{\"module\":\"services.index\",\"salon_id\":2}', '::1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.7922.34 Safari/537.36', 'EVylLVZnikIxF2MdWsshwm23DmppHzyCjf8oYXN1', NULL, 'GET', 'http://localhost/admin/tenants/2/services', '2026-08-09 12:36:39')"
          - generic [ref=e941]:
            - generic [ref=e942]:
              - text: mysql
              - generic [ref=e943]: (2.72 ms)
            - code [ref=e946]: "select count(*) as aggregate from `services` where `salon_id` = 2"
          - generic [ref=e947]:
            - generic [ref=e948]:
              - text: mysql
              - generic [ref=e949]: (6.8 ms)
            - code [ref=e952]: "select * from `services` where `salon_id` = 2 order by `name` asc limit 25 offset 0"
          - generic [ref=e953]:
            - generic [ref=e954]:
              - text: mysql
              - generic [ref=e955]: (1.37 ms)
            - code [ref=e958]: "select `id`, `name` from `service_categories` where `service_categories`.`id` in (9, 10, 20, 21, 25, 26, 27)"
          - generic [ref=e959]:
            - generic [ref=e960]:
              - text: mysql
              - generic [ref=e961]: (1.02 ms)
